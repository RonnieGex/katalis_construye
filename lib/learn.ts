...
export const LEARNING_LAYERS: LearningLayer[] = [
  ...
  },
  {
    id: "intermediate",
    title: "Intermedia",
    description: "Rentabilidad, deuda, inversion y decisiones de capital.",
    chapterNumbers: [7, 8, 9, 10, 11, 12, 13],
  },
  {
    id: "advanced",
    title: "Avanzada",
    description: "Control estrategico, valoracion, fintech y resiliencia.",
...
const CHAPTER_TOOL_MAP: Record<number, RelatedTool[]> = {
  ...
  12: [{ name: "Kit de Educacion Financiera", href: "/tools/financial-education-kit" }],
  ...
  17: [{ name: "Evaluador de Tecnologia Financiera", href: "/tools/fintech-evaluator" }],
  18: [{ name: "Calculadora de Valoracion", href: "/tools/valuation-calculator" }],
...
const CHAPTER_HINTS: Record<number, string> = {
  ...
  3: "Controla entradas/salidas para evitar tension de caja.",
  4: "Encuentra tu punto de equilibrio y valida sostenibilidad.",
  5: "Mide economia unitaria para escalar con rentabilidad.",
  ...
  9: "Monitorea margenes y retorno de la operacion.",
  10: "Alinea crecimiento, margen y estructura de capital.",
  11: "Prepara contingencias con triggers y fondos de respaldo.",
  12: "Convierte formacion financiera en resultados medibles.",
  13: "Evalua inversion con NPV/IRR antes de comprometer caja.",
  14: "Control integral con indicadores de salud financiera.",
  15: "Ajusta decisiones segun modelo de negocio.",
  16: "Gestiona riesgo cambiario y exposicion internacional.",
  17: "Elige fintech con fit real, ROI y riesgo controlado.",
  18: "Valora tu empresa con multiples y DCF.",
...
const SETUP_STEP: LearningPathStep = {
  id: "setup",
  layerId: "base",
  title: "Paso 0: Configuracion inicial",
  description: "Define moneda, tipo de cambio y modelo de negocio antes de comenzar.",
...
const CHAPTER_STEPS: LearningPathStep[] = LEARNING_LAYERS.flatMap((layer) =>
  layer.chapterNumbers.map((chapterNumber) => {
    ...
    return {
      ...
      description: CHAPTER_HINTS[chapterNumber] ?? "Aplica los conceptos del capitulo en una herramienta.",
      chapterSlug: chapterSlug(chapterNumber),
      chapterNumber,
      actionLabel: primaryTool ? `Aplicar: ${primaryTool.name}` : "Leer capitulo",
...
export function getChapterOutlineByLayer(layerId: LearningLayerId): ChapterOutlineItem[] {
  ...
  return layer.chapterNumbers.map((chapterNumber) => ({
    ...
    title: CHAPTER_TITLES[chapterNumber] ?? `Capitulo ${chapterNumber}`,
    hint: CHAPTER_HINTS[chapterNumber] ?? "Aplica este capitulo en una herramienta relacionada.",
    layerId,
